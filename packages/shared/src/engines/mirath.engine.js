...
exports.computeBlocking = computeBlocking;
exports.calculateMirath = calculateMirath;
const fraction_utils_1 = require("../utils/fraction.utils");
const madhhab_rules_1 = require("./madhhab-rules");
// ─── Internal constants ────────────────────────────────────────────────────────
const ZERO = { n: 0, d: 1 };
const ONE = { n: 1, d: 1 };
function computeBlocking(h, madhhab) {
    const rules = (0, madhhab_rules_1.getMadhhabRules)(madhhab);
 ...
 */
function umariyyatanMotherShare(h) {
    const hasChildren = h.sons > 0 || h.daughters > 0;
    ...
    if (hasChildren || hasSiblings)
        return null;
    if (h.mother === 0 || h.father === 0)
        return null;
    if (h.husband === 0 && h.wives === 0)
        return null;
    if (h.husband > 0) {
...
function computeFardShares(h, blocked, madhhab) {
    const entries = [];
    const hasChildren = h.sons > 0 || h.daughters > 0;
    ...
    if (h.daughters > 0 && h.sons === 0) {
        const frac = h.daughters === 1 ? { n: 1, d: 2 } : { n: 2, d: 3 };
        entries.push({
            key: 'daughters',
            frac,
    ...
    if (h.mother > 0) {
        ...
        if (umar) {
            ...
            const hasTwoPlusSiblings = h.fullBrothers + h.fullSisters + h.paternalHalfBrothers +
                h.paternalHalfSisters + h.maternalHalfSiblings >= 2;
            const frac = (hasChildren || hasTwoPlusSiblings)
                ? { n: 1, d: 6 }
                : { n: 1, d: 3 };
    ...
    if (h.fullSisters > 0 && !blocked.fullSisters) {
        if (h.fullBrothers === 0) {
            const frac = h.fullSisters === 1 ? { n: 1, d: 2 } : { n: 2, d: 3 };
    ...
    if (h.paternalHalfSisters > 0 && !blocked.paternalHalfSisters) {
        if (h.paternalHalfBrothers === 0) {
            ...
            const fullSisTook = fullSisFrac ? (0, fraction_utils_1.fracToDecimal)(fullSisFrac) : 0;
    ...
    if (h.maternalHalfSiblings > 0 && !blocked.maternalHalfSiblings) {
        const frac = h.maternalHalfSiblings === 1 ? { n: 1, d: 6 } : { n: 1, d: 3 };
 ...
 */
function getAsabahKey(h, blocked) {
    if (h.sons > 0)
        return 'sons';
    if (h.father > 0 && h.sons === 0 && h.daughters === 0)
        return 'father'; // pure Asabah
    if (h.father > 0 && h.daughters > 0)
        return 'father'; // father + daughters share residue
    if (h.paternalGrandfathers > 0 && !blocked.paternalGrandfathers && h.sons === 0 && h.daughters === 0)
        return 'paternalGrandfathers';
    if (h.paternalGrandfathers > 0 && !blocked.paternalGrandfathers && h.daughters > 0)
        return 'paternalGrandfathers';
    if (h.fullBrothers > 0 && !blocked.fullBrothers)
        return 'fullBrothers';
    if (h.paternalHalfBrothers > 0 && !blocked.paternalHalfBrothers)
        return 'paternalHalfBrothers';
    if (h.sonsOfFullBrothers > 0 && !blocked.sonsOfFullBrothers)
        return 'sonsOfFullBrothers';
    if (h.sonsOfPatHalfBrothers > 0 && !blocked.sonsOfPatHalfBrothers)
        return 'sonsOfPatHalfBrothers';
    if (h.paternalUncles > 0 && !blocked.paternalUncles)
        return 'paternalUncles';
    if (h.sonsOfPatUncles > 0 && !blocked.sonsOfPatUncles)
        return 'sonsOfPatUncles';
    return null;
}
 ...
 */
function applyRadd(entries, h, madhhab) {
    const rules = (0, madhhab_rules_1.getMadhhabRules)(madhhab);
    if (!rules.allowRadd) {
        // Shafii/Hanbali: no Radd to anyone, surplus goes to Bayt al-Mal
        return entries;
    }
    const spouseKeys = ['husband', 'wives'];
    ...
    let recipientSum = ZERO;
    for (const e of raddRecipients)
        recipientSum = (0, fraction_utils_1.addFrac)(recipientSum, e.frac);
    // Sum of spouse shares
    let spouseSum = ZERO;
    for (const e of spouseEntries)
        spouseSum = (0, fraction_utils_1.addFrac)(spouseSum, e.frac);
    // Remainder available for Radd = 1 - spouseSum
    const remainder = (0, fraction_utils_1.reduceFrac)({ n: spouseSum.d - spouseSum.n, d: spouseSum.d });
    // Scale each Radd recipient proportionally
    // New share_i = (original_i / recipientSum) * remainder
    return entries.map(e => {
        if (spouseKeys.includes(e.key))
            return e;
        ...
        return { ...e, frac: (0, fraction_utils_1.reduceFrac)({ n: newN, d: newD }) };
    });
}
// ─── Human-Readable Labels ────────────────────────────────────────────────────
function heirLabel(key, count) {
    const map = {
 ...
 */
function calculateMirath(input) {
    ...
    for (const e of fardEntries)
        fardTotal = (0, fraction_utils_1.addFrac)(fardTotal, e.frac);
    // ── 5. Awl or Radd correction ─────────────────────────────────────────────
    let method = 'NORMAL';
    let awlFactor;
    const fardDecimal = (0, fraction_utils_1.fracToDecimal)(fardTotal);
    if (fardDecimal > 1 + 1e-9) {
        ...
        fardEntries = fardEntries.map(e => ({
            ...e,
            frac: (0, fraction_utils_1.reduceFrac)({ n: e.frac.n * fardTotal.d, d: e.frac.d * fardTotal.n }),
        }));
    ...
    const fardPaid = netEstate * (0, fraction_utils_1.fracToDecimal)(fardTotal);
    ...
    for (const key of allKeys) {
        const count = h[key];
        if (count === 0)
            continue; // Skip heirs that are 0
        ...
        if (fardEntry && !daughtersAsAsabah) {
            ...
            const shareOfEstate = (0, fraction_utils_1.fracToDecimal)(fardEntry.frac);
            ...
            shares.push({
                ...
                count,
                shareType: 'FARD',
                fractionLabel: (0, fraction_utils_1.fracToLabel)(fardEntry.frac),
...
function buildBlockingReason(key, h, blocked) {
    switch (key) {
        ...
        case 'sonsOfFullBrothers':
            if (h.fullBrothers > 0)
                return 'Blocked by Full Brother';
            if (h.paternalHalfBrothers > 0)
                return 'Blocked by Paternal Half-Brother';
            if (h.sons > 0)
                return 'Blocked by Son';
            return 'Blocked by Father';
        case 'sonsOfPatHalfBrothers':
            if (h.sonsOfFullBrothers > 0)
                return "Blocked by Full Brother's Son";
            if (h.paternalHalfBrothers > 0)
                return 'Blocked by Paternal Half-Brother';
            return 'Blocked by Full Brother';
        case 'paternalUncles':
            if (h.sonsOfFullBrothers > 0)
                return "Blocked by Brother's Son";
            if (h.paternalHalfBrothers > 0)
                return 'Blocked by Paternal Half-Brother';
            if (h.fullBrothers > 0)
                return 'Blocked by Full Brother';
        ...
        case 'sonsOfPatUncles':
            if (h.paternalUncles > 0)
                return 'Blocked by Paternal Uncle';
